...
import { checkEmail } from '@/lib/supabase';
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="hidden sm:block">
              <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700 dark:text-gray-200'} leading-tight`}>{currentUser ? currentUser.full_name.split(' ')[0] : 'User'}</p>
              <p className={`text-[11px] ${isDarkMode ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500 dark:text-gray-400'} leading-tight`}>{currentUser?.role || 'Staff'}</p>
            </div>
          </button>
          {showUserMenu && (
            <div className="absolute right-0 mt-4 w-48 liquid-panel rounded-xl shadow-lg z-50 text-gray-800 dark:text-gray-100 border-t-0">
              <div className="p-3 border-b border-gray-200 dark:border-gray-700/50 font-semibold text-sm">{currentUser?.full_name || 'User Name'}</div>
