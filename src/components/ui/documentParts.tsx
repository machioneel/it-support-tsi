...
export function Th({ children, className = '', colSpan }: { children?: React.ReactNode; className?: string; colSpan?: number }) {
  return (
    <th colSpan={colSpan} className={`border border-gray-400 px-2 py-1 font-semibold text-left ${className}`}>
      {children}
    </th>
  );
}
...
export function Td({ children, className = '', colSpan }: { children?: React.ReactNode; className?: string; colSpan?: number }) {
  return <td colSpan={colSpan} className={`border border-gray-400 px-2 py-1 align-top ${className}`}>{children}</td>;
}
