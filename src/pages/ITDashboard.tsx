...
export default function ITDashboard() {
  ...
  const highPriority = filteredTickets.filter(t => t.priority_level === 'P1 - Critical' || t.priority_level === 'P2 - High').length;
  ...
  const veryLowPriority = 0; // Not in schema, let's keep it 0 or omit it, but we'll map to 0.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="liquid-card rounded-xl p-5">
          ...
          <div className="flex items-end justify-between h-32 px-4">
            <BarColumn height={`${Math.round((highPriority/maxPriority)*100)}%`} color="bg-red-500" label="High" value={highPriority.toString()} />
            <BarColumn height={`${Math.round((mediumPriority/maxPriority)*100)}%`} color="bg-orange-400" label="Medium" value={mediumPriority.toString()} />
            <BarColumn height={`${Math.round((lowPriority/maxPriority)*100)}%`} color="bg-blue-400" label="Low" value={lowPriority.toString()} />
            <BarColumn height={`${Math.round((veryLowPriority/maxPriority)*100)}%`} color="bg-gray-400" label="Very Low" value={veryLowPriority.toString()} />
          </div>
        </div>
...
export function PriorityBadge({ priority }: { priority: string }) {
  const getColors = () => {
    switch(priority) {
      case 'High': return 'text-red-600 border-red-200 bg-red-50/50';
      case 'Medium': return 'text-orange-600 border-orange-200 bg-orange-50/50';
      case 'Low': return 'text-blue-600 border-blue-200 bg-blue-50/50';
      default: return 'text-gray-600 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50/50';
    }
  };
